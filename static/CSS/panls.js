...
function moinsp(ecranid,prix,ecranidp) {
    ...
    //     somiu.innerHTML = parseInt(somiu.innerHTML)  - prix
    ...
    tabUsere.forEach(element => {
        let parsone = element["data"]
        if ( parseInt(parsone.position) == parseInt(ecranid) && parsone.quantiteto > 1) {
            cpt+=1
            parsone.quantiteto = parseInt(parsone.quantiteto) - 1
            console.log(parsone.quantiteto);
...
function plusp(ecranid,prix,ecranidp) {
    let tabUsere = []
    tabUsere = JSON.parse(localStorage.getItem("inscpce"))
    tabUsere.forEach(element => {
        let parsone = element["data"]
        if ( parseInt(parsone.position) == parseInt(ecranid) ) {
            parsone.quantiteto = parseInt(parsone.quantiteto) + 1
            console.log(parsone.quantiteto);
            localStorage.setItem('inscpce',JSON.stringify(tabUsere))
        }
